...
def test_all_metadata_fields_are_flattened(monkeypatch):
    ...
    tuples = repository.Repository._convert_metadata_to_list_of_tuples(data)
    # Verifies that all metadata fields as parseed by ``packaging.metadata`` are
    # correctly flattened into a list of (str, str) tuples. This does not apply
    # to the ``gpg_signature`` field, but this field is not added here as there
    # are specific tests for it.
